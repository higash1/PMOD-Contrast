...
import torch
from ..model.constant import DATASET_MAP, DATASET_POSE_ERR, DATASET_CAMERA, DATASET_IOPAINTIMG
...
class PMOD_Train_Dataset(HDF5Dataset):
    def __init__(self, h5_paths: List[str], config: str, quiet: bool = True, block_size: int = 0, use_mods: Tuple[int, int] = None,
                 visibility_filter_radius: int = 0, visibility_filter_threshold: float = 3.0, tr_err_range: float = 2.0, rot_err_range: float = 10.0, augmentation: bool = False) -> None:
        ...
        if self.augmentation is True:
            self.transforms = transforms.Compose([
                ...
                transforms.ColorJitter(brightness=0.5),  # 明るさをランダムに変更
                transforms.ToTensor()
            ])
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.augmentation is True:
            # augmentation
            items[DATASET_CAMERA] = items[DATASET_CAMERA][[2,1,0], :, :] / 255
            items[DATASET_CAMERA] = self.transforms(items[DATASET_CAMERA])[[2,1,0], :, :] * 255
            if items.get(DATASET_IOPAINTIMG) is not None:
                items[DATASET_IOPAINTIMG] = items[DATASET_IOPAINTIMG][[2,1,0], :, :] / 255
                items[DATASET_IOPAINTIMG] = self.transforms(items[DATASET_IOPAINTIMG])[[2,1,0], :, :] * 255
