...
from h5dataloader.common.structure import CONFIG_TAG_LABEL, CONFIG_TAG_LABELTAG, CONFIG_TAG_RANGE, CONFIG_TAG_SHAPE, CONFIG_TAG_TAG, CONFIG_TAG_CONFIG
...
def init_train_dataloader(args: Dict[str, Union[int, float, str, dict]]) -> DataLoader:
    ...
    train_dataset = PMOD_Train_Dataset(
        ...
        block_size=block_size, use_mods=train_use_mods,visibility_filter_radius=args[ARG_FILTER_RADIUS],
        tr_err_range=tr_err_range, rot_err_range=rot_err_range, augmentation=args[ARG_AUGMENTATION]
    )
...
def init_val_dataloader(args: Dict[str, Union[int, float, str, dict]]) -> DataLoader:
    ...
    val_dataset = PMOD_Test_Dataset(
        ...
        block_size=block_size, use_mods=val_use_mods,visibility_filter_radius=args[ARG_FILTER_RADIUS],
...
def init_eval_dataloader(args: Dict[str, Union[int, float, str, dict]]) -> DataLoader:
    ...
    eval_dataset = PMOD_Test_Dataset(
        ...
        block_size=block_size, use_mods=eval_use_mods,visibility_filter_radius=args[ARG_FILTER_RADIUS],
        tr_err_range=args.get(ARG_TR_ERROR_RANGE), rot_err_range=args.get(ARG_ROT_ERROR_RANGE)
    )
