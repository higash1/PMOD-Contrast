...
class PMOD_CONTRASTSD(nn.Module):
    def __init__(self, camera_shape: Tuple[int, int, int], num_classes: int, moldvec = None, encoder = None, easpp = None):
        """PMOD
        ...
        if encoder is not None:
            ...
            raise("encoder and easpp must be provided")

            
        encoder_out_shape: List[Tuple[int, int, int]
                                ] = self.encoder.get_out_shape()
        encoder_out_shape.append(self.easpp.get_out_shape())
        if moldvec is not None:
            self.static_enc = moldvec
        else:
    ...
    def forward(self, camera: Tensor, map_depth: Tensor) -> PMOD_CONTRASTSD_OUTPUT:
        return self.forward_adapnet(camera, map_depth)

    def forward_adapnet(self, camera: Tensor, map_depth: Tensor) -> PMOD_CONTRASTSD_OUTPUT:
        input_tensor: Tensor = torch.cat((camera, map_depth), dim=1)
        skips: List[Tensor] = self.encoder(input_tensor)

        skips.append(self.easpp(skips[-1]))
        if self.static_enc is not None:
            pmod_feature: Tensor = self.static_enc(skips[-1])
        else:
            # non relu
            pmod_feature = skips[-1]

        segmentation: ADAPNET_DECODER_OUTPUT = self.decoder_seg(skips)
        depth: ADAPNET_DECODER_OUTPUT = self.decoder_depth(skips)

        return PMOD_CONTRASTSD_OUTPUT(
            segmentation.output, self.relu(depth.output),
            segmentation.aux1, segmentation.aux2,
            pmod_feature
        )
        
class ENC_INMAP_CONTRASTSD(nn.Module):
    def __init__(self, camera_shape: Tuple[int, int, int], num_classes: int, moldvec = None, encoder = None, easpp = None):
        """PMOD
        ...
        if encoder is not None:
            ...
            self.easpp = copy.deepcopy(easpp)
            
            # freeze parameters
            for module in [self.encoder, self.easpp]:
                for param in module.parameters():
                    param.requires_grad = False
            
            if moldvec is not None:
                self.static_enc = copy.deepcopy(moldvec)
                for param in self.static_enc.parameters():
                    param.requires_grad = False
            else:
                self.static_enc = None
        else:
    ...
    def forward(self, color_inmap: Tensor, map_depth: Tensor) -> INMAP_CONTRASTSD_OUTPUT:
        return self.forward_adapnet(color_inmap, map_depth)

    def forward_adapnet(self, color_inmap: Tensor, map_depth: Tensor) -> INMAP_CONTRASTSD_OUTPUT:
        input_tensor: Tensor = torch.cat((color_inmap, map_depth), dim=1)
        skips: List[Tensor] = self.encoder(input_tensor)

        skips.append(self.easpp(skips[-1]))
        if self.static_enc is not None:
            inmap_feature: Tensor = self.static_enc(skips[-1])
        else:
            # non relu
            inmap_feature = skips[-1]

        return INMAP_CONTRASTSD_OUTPUT(
            inmap_feature
        )   

class Mold_Vector(nn.Module):
    def __init__(
        self,
        ...
        super(Mold_Vector, self).__init__()

        in_channels, in_height, in_width = in_shapes[-1]
        out_height, out_width = in_height * 16, in_width * 16
        d_channels: int = BASE_CHANNELS * 4

        # self.outshape = in_shapes[-1][0] * in_shapes[-1][1] * in_shapes[-1][2]
        
        # nontraining 
        self.a1_1_us: nn.Module = nn.Upsample(
            size=(out_height, out_width), mode=UPSAMPLE_BILINEAR, align_corners=True)
        
        self.relu = nn.ReLU(inplace=True)
        
    def forward(self, x: Tensor) -> Tensor:
        # training conv2d layer refer to aux1
        a1x1: Tensor = self.a1_1_us(x)
        a1x2: Tensor = self.relu(a1x1)
        return a1x2
