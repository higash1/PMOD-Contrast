    ...
    try:
        with torch.no_grad():
            ...
            for batch in tqdm(eval_dataloader, desc=f'{"Evaluation":11s}'):
                ...
                pred: PMOD_CONTRASTSD_OUTPUT = pmodnet(camera=in_camera, map_depth=in_map)

                # input_tensor: Tensor = torch.cat((in_camera, in_map), dim=1)
                # skips: List[Tensor] = pmodnet.encoder(input_tensor)
        
                # inmap_feature = pmodnet.easpp(skips[-1])

                # # feature heatmap
                # inmap_feature = interpolate(inmap_feature, size=(in_map.shape[2], in_map.shape[3]), mode='bilinear', align_corners=True)
                # origin_heatmap = torch.sum(inmap_feature, dim=1, keepdim=True)[0]
                # # origin_heatmap = torch.sum(pred.inmap_feature, dim=1, keepdim=True)[0]
                # heatmap = (origin_heatmap - torch.min(origin_heatmap)) / (torch.max(origin_heatmap) - torch.min(origin_heatmap))
                # heatmap = heatmap.cpu().detach().numpy().transpose(1,2,0).squeeze()
                # cv2.imwrite(os.path.join(result_dir, f'heatmap_{itr}.png'), heatmap*255)
