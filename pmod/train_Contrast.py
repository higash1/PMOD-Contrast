...
from .model.model_usfeature import PMOD_CONTRASTSD, ENC_INMAP_CONTRASTSD
...
def depth2colormap_cv(src: np.ndarray, range_min: float, range_max: float, invert:bool = False, bg_full:bool = False) -> np.ndarray:
    out_range = np.where((src < range_min) | (range_max < src))
    colormap = np.uint8(
        ((src - range_min) / (range_max - range_min)) * 255.0)

    if invert: colormap = 255 - colormap
    colormap = cv2.applyColorMap(colormap, cv2.COLORMAP_JET)
    if bg_full: colormap[out_range] = [255, 255, 255]
    else: colormap[out_range] = [0, 0, 0]
...
def parse_args() -> dict:
    ...
    parser_train.add_argument(
        f'--{arg_hyphen(ARG_AUGMENTATION)}',
        action='store_true',
        help='Using brightness augmentation.'
    )
    ...
    parser_net.add_argument(
        f'-share', f'--{arg_hyphen(SHARED_WEIGHTS)}',
        action='store_true',
        help='shared weights for the contrastive learning.'
    )
    parser_net.add_argument(
        f'-moldvec', f'--{arg_hyphen(MOLDVECTOR)}',
        action='store_true',
        help='mold vector if true using raw gt_label in contrastive triplet loss.'
    )
    ...
    parser_optim.add_argument(
        ...
        type=str, metavar='PATH', default='/workspace/pmod/config/optim-params-default.yaml',
    ...
    parser_inmap = parser.add_argument_group('InMap')
    parser_inmap.add_argument(
        f'--{arg_hyphen(ARG_FILTER_RADIUS)}',
        type=int, default=0,
        help='Filter Radius'
    )
...
def main(args: Dict[str, Union[int, float, str, dict]], workdir: str, trial: optuna.Trial = None) -> Dict[str, float]:
    ...
    if args[ARG_RESUME] is None:
        if args[SHARED_WEIGHTS] is True:
            from .model.model_usfeature import AdapnetEncoder, eAspp
            in_shape: Tuple[int, int, int] = (
                args[ARG_CAMERA_SHAPE][0] + 1, *args[ARG_CAMERA_SHAPE][1:])
            encoder = AdapnetEncoder(in_shape).to(device)
            encoder_out_shape = encoder.get_out_shape()
            easpp = eAspp(encoder_out_shape[-1]).to(device)
            
            if args[MOLDVECTOR] is True:
                from .model.model_usfeature import Mold_Vector
                moldvec = Mold_Vector(in_shapes=encoder_out_shape, out_channels=1)
            else: moldvec = None
            
            pmodnet: nn.Module = PMOD_CONTRASTSD(
            ...
            enc_inmap_contrastsd = ENC_INMAP_CONTRASTSD(
                camera_shape=args[ARG_CAMERA_SHAPE],
                num_classes=args[ARG_NUM_CLASSES],
                moldvec=moldvec,
                encoder=encoder,
            ...
            raise NotImplementedError('args[SHARED_WEIGHTS] is False')
        ...
        pmodnet.to(device)
        if args[MOLDVECTOR]:
            moldvec = pmodnet.static_enc
        enc_inmap_contrastsd = ENC_INMAP_CONTRASTSD(
            camera_shape=args[ARG_CAMERA_SHAPE],
            num_classes=args[ARG_NUM_CLASSES],
            moldvec=moldvec,
            encoder=pmodnet.encoder,
            easpp=pmodnet.easpp
        ).to(device)
    ...
    ################
    params = 0
    for model in [pmodnet, enc_inmap_contrastsd]:
    ...
    loss_Contrastive: LOSS_WEIGHT = LOSS_WEIGHT(
        LossContrast(moldvec=args[MOLDVECTOR]), args[ARG_CONTRASTIVE]).to(device)
    ...
    if args[ARG_TRAIN_DL_CONFIG] is None:
        raise ValueError('No config file for training dataloader.')
    ...
    #########################
    with open(args[ARG_TRAIN_DL_CONFIG], mode='r') as configfile:
        config_dict:Dict[str, Dict[str, Dict[str, Any]]] = json.load(configfile)
    if CONFIG_TAG_LABEL in config_dict:
        for key_label, item_label in config_dict[CONFIG_TAG_LABEL][CONFIG_TAG_CONFIG].items():
            dynamic_label_list:List = item_label[CONFIG_TAG_DYNAMIC_LABEL]['list']
    ...
    try:
        ...
        for epoch_itr in range(args[ARG_EPOCHS]):
            ...
            for batch_itr in tqdm(range(steps_per_epoch), desc=f'Epoch {epoch_num:5d}: {"Train":14s}'):
                ...
                # iopaint img
                iopaint_img: Tensor = batch[DATASET_IOPAINTIMG].to(
                    device, non_blocking=True)

                with autocast(enabled=args[ARG_AMP]):
                    pred: PMOD_CONTRASTSD_OUTPUT = pmodnet(
                        camera=in_camera, map_depth=in_map)
                    static_feature: INMAP_CONTRASTSD_OUTPUT = enc_inmap_contrastsd(
                        color_inmap=iopaint_img,map_depth=in_map)
                ...
                with autocast(enabled=args[ARG_AMP]):
                    ...
                    # loss contrastive 
                    loss_contrastive: Tensor = loss_Contrastive(pred.contrast, static_feature.contrast, gt_label.unsqueeze(dim=1),dynamic_label_list, True)
                    train_loss_dict['Contrastive'] = loss_contrastive.clone().detach()
                ...
                del loss_g, loss_seg_ce, loss_depth_l1, loss_seg_aux1_ce, loss_seg_aux2_ce, loss_contrastive
                ...
                enc_param = pmodnet.encoder.state_dict()
                easpp_param = pmodnet.easpp.state_dict()
                # copy the encoder and easpp weights to enc_inmap_contrastsd
                enc_inmap_contrastsd.encoder.load_state_dict(enc_param)
                enc_inmap_contrastsd.easpp.load_state_dict(easpp_param)
                ...
                if (step_itr % 50 == 49):
                    ...
                    img_tensor: Tensor = make_grid([
                        ...
                        torch.from_numpy(np.uint8(train_dataset.to_numpy(
                            batch[DATASET_IOPAINTIMG][0], DATASET_IOPAINTIMG)[..., ::-1] * 255).copy().transpose([2, 0, 1]))
                    ], nrow=2)
                    tb_writer.add_image(
